...
sw_split_name <- starwars %>% # Start with starwars
  separate(col = name, # separate name column
           into = c("name_1", "name_2"), # into "name_1" and "name_2"
           sep = "-") # after the first dash
...
sw_skin_sep <- starwars %>% # Start with starwars
  separate(col = skin_color, # separate skin_color column
           into = c("skin_1", "skin_2"), # into "skin_1" and "skin_2"
           sep = ",", # separated after the first comma
           extra = "merge", # merging everything after the first comma
           remove = FALSE) # retaining the original column
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create new column called model_class
        ...
        sep = "-", # separated by "-"
        remove = FALSE) # Use 'remove = FALSE' to retain original columns
...
```{r}
# Your code here: 
ymd("2000-09-21") # Write September 21, 2000 in 'ymd()' with '-'
ymd("2000/09/21") # Write September 21, 2000 in 'ymd()' with '/'
ymd(20000921) # Write September 21, 2000 in 'ymd()' with numbers only
...
```{r}
# Your code here: 
some_dates <- c(11061972, 02142007, 01301843) # Create vector of dates as numbers stored as some_dates
some_dates_converted <- mdy(some_dates) # Convert to date format with 'mdy()'
some_dates_converted # View dates
class(some_dates_converted) # Check class of some_dates_converted
...
gasol_hookshots <- lakers %>%
  filter(player == "Pau Gasol", type == "hook") %>% # Filter only "Pau Gasol" from player column and "hook" from type column
  mutate(Date = ymd(date)) %>% # Add date column to "Date" class with 'ymd()'
  mutate(month = month(Date)) %>% # Create month column with 'month()'
  count(month) # Find number of Gasol's hook shots recorded by month with 'count(month)'
...
storms_danger <- storms %>% # Start with storms
  mutate(danger = case_when( # Add a new column called danger
    status == "hurricane" ~ "high", # "high" if "hurricane"
    TRUE ~ "moderate")) # all non-specified values "moderate"
